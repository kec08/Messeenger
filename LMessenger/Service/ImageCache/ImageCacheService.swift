...
protocol ImageCacheServiceType {
    func image(forKey key: String) -> AnyPublisher<UIImage?, Never>
...
class ImageCacheService: ImageCacheServiceType {
    ...
    func imageWithMemoryCache(for ket: String) -> AnyPublisher<UIImage?, Never> {
        Future { [weak self] promiss in
            let image = self?.memoryStorage.value(for: ket)
                promiss(.success(image))
        }.eraseToAnyPublisher()
    }
